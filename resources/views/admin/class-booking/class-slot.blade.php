    ...
    @foreach ($classes_list as $item)
        <div class="col-md-2 col-12">
            @if ($item->availability_status == 'Available')
                <div class="btn btn-outline-primary btn-sm w-100 text-center px-2 class-slot-button" data-id="{{$item->id}}">
                    <span class="mb-2 fs-6 fw-bold text-light-emphasis">{{$item->availability_status}}</span>
            ...
            @else
                <button disabled class="btn btn-outline-primary border-danger btn-sm w-100 text-center px-2">
                    <span class="mb-2 fs-6 fw-bold text-danger">{{$item->availability_status == 'Not Available ' ? 'Boocked' : 'Boocked'}}</span>
                    <span class="d-flex pt-2"> {{\Carbon\Carbon::parse($item->start_time)->format('h:i A') }} - {{\Carbon\Carbon::parse($item->end_time)->format('h:i A') }} </span>
                </button>
            @endif
        </div>
